...
async function loadDashboardData() {
  try {
    const [projectsSnap, bookingsSnap, inquiriesSnap, reviewsSnap] =
      await Promise.all([
    ...
    recentActivity.innerHTML =
      '<p class="text-slate-400 text-sm">Dashboard loaded successfully</p>';
  } catch (error) {
...
async function loadProjects() {
  ...
  list.innerHTML = `
    <div class="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p class="text-slate-400 mb-6">Start by adding your first project</p>
      <button onclick="document.getElementById('addProjectBtn').click()" 
        class="px-6 py-2 bg-cyan-500 hover:bg-cyan-600 rounded-lg font-semibold text-white">
        + Add Project
      </button>
    </div>
  `;
...
` : ""}
          ...
          <div class="flex gap-2 mt-3">
            <span class="px-3 py-1 bg-purple-500/20 text-purple-300 text-xs rounded">
              ${data.category || "Uncategorized"}
            </span>
            ${
              data.link
                ? `<a href="${data.link}" target="_blank" class="px-3 py-1 bg-cyan-500/20 text-cyan-300 text-xs rounded hover:bg-cyan-500/40 transition">View Live</a>`
                : ""
            }
          </div>
        </div>
...
async function loadServices() {
  try {
    ...
    if (snapshot.empty) {
      list.innerHTML =
        '<div class="text-slate-400 text-sm">No services yet. Add your first service!</div>';
      return;
    }
    ...
    list.innerHTML = snapshot.docs
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-lg p-6 flex justify-between items-start">
        <div class="flex-1">
          ...
          ${
            data.price
              ? `<p class="text-cyan-400 font-bold mt-2">₹${data.price}</p>`
              : ""
          }
        </div>
        <div class="flex gap-2 ml-4">
...
async function loadBookings() {
  try {
    ...
    if (snapshot.empty) {
      list.innerHTML =
        '<div class="text-slate-400 text-sm">No booking requests yet.</div>';
      return;
    }
...
async function loadInquiries() {
  try {
    ...
    if (snapshot.empty) {
      list.innerHTML =
        '<div class="text-slate-400 text-sm">No inquiries yet.</div>';
      return;
    }
...
async function loadReviews() {
  try {
    ...
    if (snapshot.empty) {
      list.innerHTML =
        '<div class="text-slate-400 text-sm">No reviews yet.</div>';
      return;
    }
...
function setupModalBackgroundClose() {
  ["projectModal", "serviceModal"].forEach((id) => {
